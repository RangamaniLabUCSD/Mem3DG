...
x = np.reshape(init_mesh.vertices,np.size(init_mesh.vertices),order='F') 
x = x + np.random.uniform(-0.001,0.001,np.size(init_mesh.vertices))
velo = np.zeros(x.size)
acc = np.zeros(x.size)
...
for i in range(int(T/h)):    
    ...
    #an4 = damping_force(init_mesh,gamma,x,velo)
    ...
    velo_new = velo + (an) * h
    x_new = x + velo_new * h + an * h**2 / 2
    
    x = x_new
    velo = velo_new
    acc = an
    
    v_new = np.reshape(x,init_mesh.vertices.shape,order = 'F')
