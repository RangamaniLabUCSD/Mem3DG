...
double getTotalEnergy(Force &f) {
  ...
  // same for all time pt for (size_t i = 0; i < positions.rows(); i++) { 	for
  //(size_t j = 0; j < positions.rows(); j++) { 		Eb += positions.row(i) * A *
  //positions.transpose().col(j);
  //	}
  //}
  ...
  k_dH_sqrd =
      f.P.Kb * (difference.array() * difference.array()).colwise().sum();
