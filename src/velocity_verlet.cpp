  ...
  namespace integration {
    ...
    void velocityVerlet(Force &f, double dt, double total_time,
      ...
      for (int i = 0; i <= total_time / dt; i++) {
        ...
        double normalStaticForceMax =
            rowwiseDotProduct(staticForce,
        ...
        if ((i % nSave == 0) || (i == int(total_time / dt))) {
          ...
          if ((dVolume < tolerance) && (dArea < tolerance) &&
              (dBE < tolerance)) {
            std::cout << "\n"
        ...
        if (i == int(total_time / dt)) {
          std::cout << "\n"
                    << "Fail to converge in given time and Exit. Past data saved to " + outputDir << std::endl;
          getSummaryLog(f, dt, i * dt, dArea, dVolume, dBE, BE, outputDir);
        }
