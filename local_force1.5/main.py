...
vol0 = 1 * 4/3 * np.pi* R**3
kt = 0.0002
alpha = 2
conc = 25 

assembler = pymesh.Assembler(init_mesh)
M = assembler.assemble("mass")
M_1 = sp_inv(assembler.assemble("mass"))
L = assembler.assemble("laplacian")

def external_force(init_mesh,x,conc,sigma):
    v = np.reshape(x,init_mesh.vertices.shape,order='F')
    node = 0
    X = v[node]
    
    Hn = M_1 @ L @ v #spsolve(M,L @ v) 
    H = np.linalg.norm(Hn,axis = 1)
    n = Hn / H[:,None]
    
    v_temp = np.reshape(x,init_mesh.vertices.T.shape,order='C')
    distance = np.reshape(v_temp - X[:,None],v.size,order='C')
    distance = np.reshape(distance,init_mesh.vertices.shape,order='F')
    distance = np.linalg.norm(distance,axis = 1)
    
    sigma = np.max(distance)/conc
    gaussian = alpha/(sigma*(np.pi * 2)**(0.5)) * np.exp(-distance**2/(2*sigma**2))
    f = np.outer(gaussian,n[node])
    f = np.reshape(f,np.size(v),order='F') 
    
    return f
    
def bending_force(init_mesh,x,H0,Kb,M,M_1,L):
    v = np.reshape(x,init_mesh.vertices.shape,order='F')
    ...
    for i in range(init_mesh.vertices.shape[0]):
        ...
        for j in range(adj_vertex.size):
            ...
            if connectivity_matrix[i][adj_vertex[j]] == 0:
                random_var[i][adj_vertex[j]] = np.random.normal(0,1)
...
x_past = np.reshape(np.load("vertices.npy"),np.size(init_mesh.vertices),order='F') 
...
for i in range(int(T/h)):   
    ...
    an1,H = bending_force(init_mesh,x,np.zeros(np.size(init_mesh.vertices.shape[0])),Kb,M,M_1,L)
    ...
    an6 = external_force(init_mesh,x,conc,sigma)
    print("fb: ",np.linalg.norm(an1),
          " fs: ",np.linalg.norm(an2),
          " fp: ",np.linalg.norm(an3),
          " fd: ",np.linalg.norm(an4),
          " fx: ",np.linalg.norm(an5),
          " fex: ",np.linalg.norm(an6))
    
    an = an1+ an2 + an3 +an4 + an6
    
    grad_norm = (np.linalg.norm(an1+an2+an3+an6))
    print('i = ',i,'',grad_norm)
